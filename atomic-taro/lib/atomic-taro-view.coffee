...
class AtomicTaroView# extends ScrollView
  ...
  constructor: (plainCodeEditor, {@fpath, @protocol}) ->
    ...
    (@addSegmentBox(chunk, block_pane)) for chunk in segs
    @addJqueryListeners()
  ...
  addSegmentBox: (segment, block_pane) ->
    model_editor = segment.getEditor()
    codeTitle = segment.getTitle()
    ...
    if segment instanceof SharedFunctionSegment
      @addQuestionBox_sharedFunctions(codeTitle, headerContainer)
    else
      @addQuestionBox_header(codeTitle, headerContainer)
    blockDiv.appendChild(headerContainer)
    #----------editor-------------
    editorContainer = @addEditorDiv(model_editor, blockDiv)
